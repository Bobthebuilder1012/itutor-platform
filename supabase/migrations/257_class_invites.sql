-- =====================================================
-- MIGRATION 257: TEACHER ACTIVATION — CLASS INVITES
-- =====================================================
-- The schema behind lib/classInvites/*. Written to that code's contract:
-- ClassInviteRow in types.ts is the column list, and the queries in claim.ts,
-- fulfil.ts and counting.ts are what the indexes below serve.
--
-- One row per invited PERSON, not per link. An addressed invitation is written
-- when the teacher sends it; a shared class link has no row until a real person
-- signs up through it, at which point recordLinkArrival() writes one with
-- source='link' and the address finally known. That is why invitee_email is NOT
-- NULL: every row here describes somebody real.
--
...
-- branches. Migration numbers are not globally unique in this repo — 250 and
-- 251 already mean different things on staging and union-staging-payment — so
-- check every branch before claiming the next one.
-- =====================================================
...
CREATE TABLE IF NOT EXISTS public.class_invites (
  ...
  CONSTRAINT class_invite_not_self       CHECK (user_id IS NULL OR user_id <> tutor_id),
  CONSTRAINT class_invite_not_self_joined CHECK (joined_student_id IS NULL OR joined_student_id <> tutor_id)
);
...
CREATE INDEX IF NOT EXISTS idx_class_invites_claimant
  ON public.class_invites (user_id, status, created_at);
CREATE INDEX IF NOT EXISTS idx_class_invites_email
  ON public.class_invites (invitee_email, status, created_at);
-- The teacher's dashboard, and the launch-goal count.
CREATE INDEX IF NOT EXISTS idx_class_invites_tutor
  ON public.class_invites (tutor_id, status);
CREATE INDEX IF NOT EXISTS idx_class_invites_group
  ON public.class_invites (group_id);
...
DROP POLICY IF EXISTS "Tutor reads own class invites" ON public.class_invites;
CREATE POLICY "Tutor reads own class invites" ON public.class_invites
