  ...
  async function performSearch() {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        console.log('Parent search - Tutors:', profilesWithRatings.map(p => ({
          name: p.full_name,
      ...
      <div className="px-4 py-6 sm:px-0 max-w-7xl mx-auto bg-gradient-to-br from-transparent via-green-50/20 to-blue-50/20 rounded-3xl">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            ...
            {!loading && results.length > 0 && ' • Verified iTutors shown first'}
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {results.map((tutor) => (
              <button
                key={tutor.id}
                onClick={() => router.push(`/parent/tutors/${tutor.id}`)}
