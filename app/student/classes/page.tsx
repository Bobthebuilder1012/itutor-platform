...
function EmptyState({ query }: { query: string }) {
  return (
    <div className="col-span-full flex flex-col items-center py-20 text-center gap-3">
      ...
      <p className="font-semibold text-ink">No classes found{query ? ` for "${query}"` : ''}</p>
      <p className="text-sm text-muted-foreground max-w-xs">
        Try a different subject or clear the search to browse all classes.
      </p>
    </div>
  );
}
    ...
    >
      ...
      >
        ...
        <button
          ...
          className={cn(
            'absolute top-3 right-3 size-8 rounded-full grid place-items-center transition z-20',
            saved ? 'bg-white/90 text-coral' : 'bg-white/30 text-white hover:bg-white/50',
          )}
          aria-label={saved ? 'Unsave class' : 'Save class'}
        >
          <Heart className={cn('size-4', saved && 'fill-coral')} />
        </button>
      ...
      <div className="flex-1 flex flex-col p-4 gap-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {c.kind === 'recurring-1on1' && (
            <span className="inline-flex items-center gap-1 bg-lavender text-ink text-[11px] font-medium px-2 py-0.5 rounded-full">
              🔄 Recurring 1:1
            </span>
          )}
        </div>
        ...
        {c.kind === 'group' && c.seatsTotal !== undefined && c.seatsTaken !== undefined && (
          <div className="space-y-1">
            ...
            <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
              <div
                className={cn('h-full rounded-full transition-all', isLowStock ? 'bg-coral' : 'bg-brand')}
...
export default function BrowseClassesPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-ink">Browse Classes</h1>
        <p className="text-muted-foreground text-sm mt-1">
          Join a recurring group class or a weekly 1:1 with a verified Caribbean tutor.
        </p>
      </div>
