...
export async function GET(req: NextRequest) {
  ...
  let query = db
    ...
    .select('id, tutor_id, author_id, session_id, body, stars, like_count, dislike_count, edited_at, hidden_at, deleted_at, created_at, author:profiles!tutor_profile_comments_author_id_fkey(id, full_name, display_name, avatar_url)', { count: 'exact' })
    .eq('tutor_id', tutorId)
    .is('deleted_at', null)
    .is('hidden_at', null)
  ...
  const { data: replies } = commentIds.length
    ? await db
        ...
        .eq('target_type', 'tutor_profile_comment')
