...
const HOLD_REASON_LABELS: Record<string, string> = {
  student_reported_tutor_no_show: 'No-show claim',
...
export default function AdminPayoutCasesPage() {
  ...
  async function loadCases() {
    ...
    // The API only supports one status filter at a time — use 'open' as default for combined
    const url = `/api/admin/payout-cases?page=${page}&page_size=${PAGE_SIZE}` +
      (statusFilter !== 'all' && !statusFilter.includes(',') ? `&status=${statusFilter}` : '');
    try {
    ...
    <DashboardLayout role="admin" userName="Admin">
      <div className="max-w-6xl mx-auto space-y-6 p-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Payout Cases</h1>
            <p className="text-sm text-gray-500 mt-0.5">
              Held payouts awaiting admin decision. {total > 0 && `${total} total case${total === 1 ? '' : 's'}.`}
            </p>
          </div>
          {openCount > 0 && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-100 text-amber-800 text-sm font-semibold">
              <AlertCircle className="size-4" /> {openCount} need{openCount === 1 ? 's' : ''} action
            </span>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            <option value="open,under_review">Active (open + under review)</option>
            <option value="open">Open only</option>
            <option value="under_review">Under review only</option>
            <option value="resolved_release_to_tutor">Released to tutor</option>
            <option value="resolved_refund_student">Refunded to student</option>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-16 gap-2 text-gray-400">
            <Loader2 className="size-5 animate-spin" /> <span className="text-sm">Loading cases…</span>
          </div>
        ) : cases.length === 0 ? (
          ...
          <div className="rounded-xl border border-gray-200 divide-y divide-gray-100 overflow-hidden bg-white">
            {cases.map((c) => (
              <CaseRow key={c.id} payoutCase={c} onResolved={onResolved} />
            ))}
          </div>
        )}
  ...
  return (
    <div>
      <button
        ...
        className={cn('w-full text-left px-5 py-4 flex items-center gap-4 hover:bg-gray-50 transition',
          isActive && 'bg-amber-50/40')}
      >
        {/* Status chip */}
        <span className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider shrink-0', sc.cls)}>
          {sc.icon} {sc.label}
        </span>
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-gray-900 truncate">
            ...
            {c.tutor?.email && <span className="ml-1.5 text-gray-400 font-normal text-xs">{c.tutor.email}</span>}
          </div>
          <div className="text-xs text-gray-500 mt-0.5">
            {HOLD_REASON_LABELS[c.hold_reason] ?? c.hold_reason}
            {c.session && ` · Session ${fmtDate(c.session.scheduled_start_at)}`}
            {c.claimant && ` · Claim by ${c.claimant.full_name}`}
          </div>
        </div>
      ...
      {expanded && (
        <div className="border-t border-gray-100 bg-gray-50 px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            ...
            <DetailCell label="Ledger status" value={c.payout_ledger?.status ?? 'No ledger'} />
            <DetailCell label="Held amount" value={fmtTTD(c.payout_ledger?.amount_ttd ?? null)} />
            <DetailCell label="Tutor" value={c.tutor?.full_name ?? '—'} />
            <DetailCell label="Claimant" value={c.claimant?.full_name ?? '—'} />
          ...
          {c.noshow_claim && (
            <div className="text-xs text-gray-600 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
              ...
              {c.noshow_claim.admin_verdict && <>, verdict <strong>{c.noshow_claim.admin_verdict}</strong></>}
            </div>
          )}
          ...
          {isActive && (
            <ResolveForm caseId={c.id} heldAmount={c.payout_ledger?.amount_ttd ?? null} onResolved={onResolved} />
          )}
        </div>
      )}
    </div>
...
function ResolveForm({
  ...
  async function resolve() {
    ...
    try {
      const body: any = { action, admin_notes: notes || undefined };
      ...
      if (json.warning) {
        setSuccess(`Refund issued but DB sync pending: ${json.warning}. Retry to close the case.`);
      } else {
  ...
  return (
    <div className="border border-gray-200 rounded-xl bg-white p-4 space-y-3">
      <div className="text-xs font-semibold text-gray-700 uppercase tracking-wider">Resolve this case</div>

      {/* Action selector */}
      <div className="flex flex-wrap gap-2">
        {([
          ['release_to_tutor', 'Release to tutor', 'bg-emerald-100 text-emerald-800 border-emerald-300'],
          ['refund_student',   'Refund student',   'bg-rose-100 text-rose-800 border-rose-300'],
          ['partial_refund',   'Partial refund',   'bg-purple-100 text-purple-800 border-purple-300'],
      ...
      <div className="text-xs text-gray-500">
        {action === 'release_to_tutor' && 'The held payout will be restored to the tutor\'s pending balance. No refund is issued.'}
        {action === 'refund_student' && `The full held amount (${fmtTTD(heldAmount)}) will be refunded to the student via LuniPay. Payout is reversed.`}
        {action === 'partial_refund' && 'Specify how much to refund to the student and how much to release to the tutor. Amounts are validated independently.'}
      </div>
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        <button
          ...
          className={cn(
            'px-4 py-2 rounded-lg text-sm font-semibold text-white transition disabled:opacity-50',
            action === 'release_to_tutor' ? 'bg-emerald-600 hover:bg-emerald-700' :
            action === 'refund_student'   ? 'bg-rose-600 hover:bg-rose-700' :
                                            'bg-purple-600 hover:bg-purple-700'
          )}
        >
          {working ? <Loader2 className="size-4 animate-spin" /> : `Confirm — ${
            action === 'release_to_tutor' ? 'Release to Tutor' :
            action === 'refund_student'   ? 'Refund Student' :
                                            'Partial Refund'
          }`}
        </button>
      </div>
