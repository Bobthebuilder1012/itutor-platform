-- ===============================================
-- PUSH NOTIFICATION TEST FOR JOVANMR (V2)
-- ===============================================
...
-- Step 2: Clean up old test data and create new test booking/session
DO $$
DECLARE
  v_jovan_id uuid;
  v_tutor_id uuid;
...
BEGIN
  ...
  -- Clean up old test sessions (will cascade delete from bookings due to foreign key)
  DELETE FROM sessions
  WHERE student_id = v_jovan_id 
    AND notes::text LIKE '%TEST SESSION%'
    AND created_at > now() - interval '1 day';

  -- Clean up old test bookings without sessions
  DELETE FROM bookings
  WHERE student_id = v_jovan_id 
    AND student_notes LIKE '%TEST BOOKING%'
    AND created_at > now() - interval '1 day';
  ...
  RAISE NOTICE 'Test booking created: %', v_booking_id;
...
SELECT 
  ...
  '' as blank5,
  '♻️  NOTE: This script can be run multiple times safely' as note1,
  '- Old test data is cleaned up before creating new test session' as note2;
