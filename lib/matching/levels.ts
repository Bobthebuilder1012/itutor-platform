/**
 * Level normalisation, shared by every matching surface.
 *
...
/** The seven levels the platform matches on. The canonical internal vocabulary. */
...
export function normaliseClassLevel(raw: string | null | undefined): CanonicalLevel[] {
  ...
  console.warn(
    `[matching] Unrecognised groups.form_level: ${JSON.stringify(raw)} ` +
      `(canonicalised: ${JSON.stringify(value)}). Class excluded from matching. ` +
      `Add a rule in lib/matching/levels.ts.`
  );
  return [];
}
