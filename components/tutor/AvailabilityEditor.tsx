...
// Reusable weekly-availability editor. Extracted from app/tutor/availability so
// it can also live inside the tutor "My Business" page. Writes tutor_availability_rules
// via bookingService (delete-all-then-upsert), and fires notifyCompletionUpdated()
// + onSaved so any completion gate / parent stays in sync.
...
function rulesToSlots(rules: TutorAvailabilityRule[]): Slot[] {
  ...
  for (const rule of rules) {
    ...
    for (let h = startHour; h < endHour; h++) slots.push({ day: rule.day_of_week, hour: h });
  }
  return slots;
}
...
export default function AvailabilityEditor({ tutorId, onSaved }: { tutorId: string; onSaved?: () => void }) {
  ...
  async function loadRules(id: string) {
    setDataLoading(true);
    try {
      const rules = await getTutorAvailabilityRules(id);
  ...
  async function save() {
    ...
    try {
      ...
      await Promise.all(
        newRules.map((r) =>
          upsertAvailabilityRule({ tutor_id: tutorId, ...r, slot_minutes: 60, buffer_minutes: 0, is_active: true })
        )
      );
      await loadRules(tutorId);
      setSaved(true);
  ...
  if (dataLoading) {
    return <div className="min-h-[240px] flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-brand" /></div>;
  }
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
        <div className="flex items-center justify-between gap-2">
          <p className="text-xs text-muted-foreground">Click slots to mark when you&apos;re available. Timezone: AST (Trinidad).</p>
          <button onClick={copyMonToWeekdays} className="shrink-0 inline-flex items-center gap-1 text-xs font-semibold text-brand-deep hover:bg-brand/10 px-2 py-1 rounded">
