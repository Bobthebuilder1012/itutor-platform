/**
 * Migration runner for 197_stripe_provider.sql (staging).
 *
 * Applies the Stripe provider migration against the Supabase Postgres
 * pointed to by NEXT_PUBLIC_SUPABASE_URL in .env.local.
 *
 * SAFE TO RE-RUN: 197 uses only idempotent constructs
 * (ADD COLUMN IF NOT EXISTS / CREATE TABLE IF NOT EXISTS /
 * CREATE INDEX IF NOT EXISTS) and is wrapped in BEGIN/COMMIT.
 *
 * Run: npx ts-node scripts/apply-migration-197.ts
 *      npx ts-node scripts/apply-migration-197.ts --dry-run   (probe only)
 *
...
const MIGRATION = '197_stripe_provider.sql';

/** Read-only state probe — tells us what's already there before we touch anything. */
const PROBE = `
  SELECT
    (SELECT to_regclass('public.stripe_webhook_events') IS NOT NULL)        AS stripe_webhook_events_table,
    (SELECT EXISTS(SELECT 1 FROM information_schema.columns
       WHERE table_name='payments' AND column_name='stripe_payment_intent_id')) AS payments_intent_col,
    (SELECT EXISTS(SELECT 1 FROM information_schema.columns
       WHERE table_name='payments' AND column_name='charged_processing_fee_ttd')) AS payments_charged_fee_col,
    (SELECT EXISTS(SELECT 1 FROM information_schema.columns
       WHERE table_name='payments' AND column_name='fee_variance_ttd'))     AS payments_variance_col,
    (SELECT to_regclass('public.payout_cases') IS NOT NULL)                 AS payout_cases_dep,
    (SELECT EXISTS(SELECT 1 FROM pg_proc
       WHERE proname='complete_booking_payment'))                           AS complete_booking_payment_dep;
`;
...
async function main() {
  ...
  const ok =
    row.stripe_webhook_events_table &&
    row.payments_intent_col &&
    row.payments_charged_fee_col &&
    row.payments_variance_col;
